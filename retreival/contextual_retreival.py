...
def contextual_retrieval(query,user_history=[], top_k=5):
    context = " ".join(user_history)
    contextualized_query = query + " " + context
    
    return dense_retrieval(contextualized_query, top_k)

for text, score in contextual_retrieval("matching", ["neural", "embeddings"]):
